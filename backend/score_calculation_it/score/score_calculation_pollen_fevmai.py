...
score_columns_pollen = ["score_pollen_arbres_weight_fevmai", "score_pollen_parcs_prop_fevmai"]
...
def all_prop(input_path, params, output_path):
    ...
    for dataname, dataprops in params.items():
        ...
        edges[f"{dataname}_fevmai"] = data[dataname]
...
def all_score_edges(input_path, output_path, params):
    ...
    for data_name, data_param in params.items():
        ...
        default_edges[f"score_pollen_{data_name}_fevmai"] = data[data_name].apply(data_param["fn_cont"])
...
def one_score_edges(input_path, output_path, params, key):
    ...
    default_edges[f"score_pollen_{key}_fevmai"] = data[key].apply(params[key]["fn_cont"])
...
def score_pollen(input_path, output_path):
    ...
    edges["pollen_score_fevmai"] = edges["score_distance_pollen_fevmai"].apply(lambda x: round(slope*x+origin_ordinate, 2))
...
def create_graph_pollen(graph_path, edges_buffered_path, graph_output_path):
    ...
    graph_e["total_score_pollen_fevmai"] = edges_buffered["total_score_pollen_fevmai"]
    graph_e["score_distance_pollen_fevmai"] = edges_buffered["score_distance_pollen_fevmai"]
    graph_e["pollen_score_fevmai"] = edges_buffered["pollen_score_fevmai"]
...
create_graph_pollen(metrop_network_bouding_path, edges_buffer_total_score_distance_pollen_fevmai_path, final_network_pollen_fevmai_path)
