...
export const MainContextProvider = ({ children }) => {
    ...
    useEffect(() => {
        async function fetchFreshnessLayers() {
            ...
            const fetchFreshPlaces = await axios.get(
                'https://download.data.grandlyon.com/files/rdata/sortons_au_frais/equipements_frais.json'
            );
            const freshplaces = {
                id: 'batiments_frais',
                geojson: {
    ...
    useEffect(() => {
        if (userPosition) {
            axios
                .get(`https://api-adresse.data.gouv.fr/reverse/?lon=${userPosition[1]}&lat=${userPosition[0]}`)
                .then(response => {
