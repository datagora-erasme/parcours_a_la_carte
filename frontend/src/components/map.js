...
function MapFreshness({ setZoomToUserPosition, zoomToUserPosition, radius, selectedStartAddress, showCircle }) {
    ...
    if (selectedStartAddress && showCircle) {
        ...
        map.eachLayer(layer => {
            if (
                layer.options &&
                (layer.options.id === 'freshnessAroundUser' || layer.options.id === 'userPosition' || layer.options.id === 'doughnuts')
            ) {
...
function Map() {
    ...
    return (
        <div>
            ...
            <MapContainer
                id="map"
                center={[45.76309302427536, 4.836502750843036]}
                minZoom={11}
                zoom={13}
            ...
            >
                ...
                />
                <ZoomItinerary
                    zoomToItinerary={zoomToItinerary}
                    setZoomToItinerary={setZoomToItinerary}
                ...
                {currentItinerary &&
                    Object.entries(currentItinerary).map(([key, itinerary], index) => {
                        return (
                            <GeoJSON
                                key={Math.random()}
                ...
                {filteredFreshnessFeatures.length !== 0 &&
                    filteredFreshnessFeatures.map(data => {
                        if (data.length !== 0) {
                                        ...
                                        {data.map((dta, i) => {
                                            ...
                                            return (
                                                <Marker
                                                    ...
                                                    icon={new L.icon({ ...markerOption, className: 'pointer-cursor' })}
                                                    eventHandlers={{
                ...
                {filteredItinerariesFeatures.length !== 0 &&
                    filteredItinerariesFeatures.map(data => {
                            ...
                            if (dataType === 'MultiPolygon' || dataType === 'Polygon') {
                                return (
                                    <GeoJSON
                                        ...
                                        data={data.geojson}
                                        style={getColor}
                                        onEachFeature={showDetailsPopupPolygon}
                                    />
                                );
                            } else if (dataType === 'Point') {
                                const markerOption = data.markerOption;
